...
from model_nin_tap_quant import NiNTapQuant, get_nin_layer_names
from model_mobilenetv1_tap_quant import MobileNetV1TapQuant, get_mobilenetv1_layer_names
...
# AlexNet的层（5个卷积层 + 2个FC层，共7层）
ALEXNET_LAYERS = ["conv1", "conv2", "conv3", "conv4", "conv5", "fc1", "fc2"]
# 为了兼容性保留旧名称
ALEXNET_CONV_LAYERS = ALEXNET_LAYERS

# LeNet-5的层（2个卷积层 + 2个FC层，共4层）
LENET5_LAYERS = ["conv1", "conv2", "fc1", "fc2"]

# NiN的层
NIN_LAYERS = get_nin_layer_names()

# MobileNetV1的层
MOBILENETV1_LAYERS = get_mobilenetv1_layer_names()
...
# ------------------ VGG19 Teacher/Student ------------------
def _build_teacher_int8_vgg19(args):
    """构建VGG19 Teacher (INT8, CPU)"""
    torch.backends.quantized.engine = args.backend
    m = VGG19TapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
    m.train(); prepare_qat(m, inplace=True)
    sd = torch.load(os.path.join(args.out, "vgg19_qat_preconvert.pth"), map_location="cpu")
    m.load_state_dict(sd, strict=True)
    m.eval()
    qmodel = convert(m, inplace=False)  # 真 INT8
    qmodel.eval()                       # 常驻 CPU
...
def _build_student_fq_vgg19(args):
    """构建VGG19 Student (FakeQuant, CUDA)"""
    torch.backends.quantized.engine = args.backend
    m = VGG19TapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
...
# ------------------ AlexNet Teacher/Student ------------------
def _build_teacher_int8_alexnet(args):
    """构建AlexNet Teacher (INT8, CPU)"""
    torch.backends.quantized.engine = args.backend
    m = AlexNetTapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
    m.train(); prepare_qat(m, inplace=True)
    sd = torch.load(os.path.join(args.out, "alexnet_qat_preconvert.pth"), map_location="cpu")
    m.load_state_dict(sd, strict=False)  # strict=False以兼容量化参数格式差异
    m.eval()
    qmodel = convert(m, inplace=False)  # 真 INT8
    qmodel.eval()                       # 常驻 CPU
    return qmodel

def _build_student_fq_alexnet(args):
    """构建AlexNet Student (FakeQuant, CUDA)"""
    torch.backends.quantized.engine = args.backend
    m = AlexNetTapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
...
# ------------------ LeNet-5 Teacher/Student ------------------
def _build_teacher_int8_lenet5(args):
    """构建LeNet-5 Teacher (INT8, CPU)"""
    torch.backends.quantized.engine = args.backend
    m = LeNet5TapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
    m.train(); prepare_qat(m, inplace=True)
    sd = torch.load(os.path.join(args.out, "lenet5_qat_preconvert.pth"), map_location="cpu")
    m.load_state_dict(sd, strict=True)
    m.eval()
    qmodel = convert(m, inplace=False)  # 真 INT8
    qmodel.eval()                       # 常驻 CPU
...
def _build_student_fq_lenet5(args):
    """构建LeNet-5 Student (FakeQuant, CUDA)"""
    torch.backends.quantized.engine = args.backend
    m = LeNet5TapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
...
# ------------------ NiN Teacher/Student ------------------
def _build_teacher_int8_nin(args):
    """构建NiN Teacher (INT8, CPU)"""
    torch.backends.quantized.engine = args.backend
    m = NiNTapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
    m.train(); prepare_qat(m, inplace=True)
    sd = torch.load(os.path.join(args.out, "nin_qat_preconvert.pth"), map_location="cpu")
    m.load_state_dict(sd, strict=True)
    m.eval()
    qmodel = convert(m, inplace=False)  # 真 INT8
    qmodel.eval()                       # 常驻 CPU
...
def _build_student_fq_nin(args):
    """构建NiN Student (FakeQuant, CUDA)"""
    torch.backends.quantized.engine = args.backend
    m = NiNTapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
...
# ------------------ MobileNetV1 Teacher/Student ------------------
def _build_teacher_int8_mobilenetv1(args):
    """构建MobileNetV1 Teacher (INT8, CPU)"""
    torch.backends.quantized.engine = args.backend
    m = MobileNetV1TapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
    m.train(); prepare_qat(m, inplace=True)
    sd = torch.load(os.path.join(args.out, "mobilenetv1_qat_preconvert.pth"), map_location="cpu")
    m.load_state_dict(sd, strict=True)
    m.eval()
    qmodel = convert(m, inplace=False)  # 真 INT8
    qmodel.eval()                       # 常驻 CPU
...
def _build_student_fq_mobilenetv1(args):
    """构建MobileNetV1 Student (FakeQuant, CUDA)"""
    torch.backends.quantized.engine = args.backend
    m = MobileNetV1TapQuant(num_classes=10)
    m.eval(); m.fuse_model()
    m.qconfig = get_default_qat_qconfig(args.backend)
...
@torch.no_grad()
def _collect_histograms(teacher, loader, num_batches=8, layers=None):
    """
    ...
    Args:
        teacher: 量化后的教师模型
        loader: 数据加载器
        num_batches: 收集多少个batch
        layers: 需要收集的层名列表（如None则使用CONV_LAYERS）
    """
    if layers is None:
        layers = CONV_LAYERS
    
    # 为每层创建直方图，支持卷积层和FC层不同的前缀
    h = {}
    for lyr in layers:
        if lyr == 'fc':
            h["fc.out"] = torch.zeros(256, dtype=torch.long)
        elif lyr.startswith('fc'):
            # FC层使用 classifier.{name}.out 前缀
            h[f"classifier.{lyr}.out"] = torch.zeros(256, dtype=torch.long)
        else:
            # 卷积层使用 block_output.{name} 前缀
            h[f"block_output.{lyr}"] = torch.zeros(256, dtype=torch.long)
...
def teacher_forward_with_scales(teacher, images_cpu):
    ...
    # 构建edits字典，支持不同模型
    edits = {}
    model_name = teacher.__class__.__name__
    
    if 'VGG19' in model_name:
        # VGG19: 16 conv layers + 1 fc layer
        for k in VGG19_CONV_LAYERS:
            edits[f"block_output.{k}"] = make_sniffer(k)
        # VGG19 FC layer records as 'fc.out'
        edits["fc.out"] = make_sniffer("fc")
        
    elif 'AlexNet' in model_name:
        # AlexNet: 5 conv layers + 2 fc layers (fc1, fc2)
        for k in ["conv1", "conv2", "conv3", "conv4", "conv5"]:
            edits[f"block_output.{k}"] = make_sniffer(k)
        for k in ["fc1", "fc2"]:
            edits[f"classifier.{k}.out"] = make_sniffer(k)
            
    elif 'LeNet5' in model_name:
        # LeNet5: 2 conv layers + 2 fc layers
        ...
        # MobileNetV1: 27 conv layers (1 standard + 13*2 DW/PW)
        for k in MOBILENETV1_LAYERS:
            edits[f"block_output.{k}"] = make_sniffer(k)
            
    else:
        # Default (VGG16): 13 conv layers
...
def train_supervised(args):
    ...
    hists = _collect_histograms(teacher, test_loader, num_batches=calib_batches, layers=target_layers)
    ...
    tri_map: Dict[str, TriApproxINT8] = {lyr: TriApproxINT8(beta=1.2).to(device) for lyr in target_layers}
    for lyr, mod in tri_map.items():
        if lyr.startswith('fc') or lyr.startswith('classifier'):
             # FC层直方图key处理
             if lyr == 'fc': h_key = "fc.out"
             else: h_key = f"classifier.{lyr}.out"
        else:
             h_key = f"block_output.{lyr}"
